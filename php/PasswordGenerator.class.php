...
class PasswordGenerator{
	private $config_NumberOfWords = 3;
	private $config_MaxNumberOfDigitBetweenWords = 2;
	private $config_DigitsAfterLastWord = true;

	public $generatedPassword = '';
	private $safeEnglishWords = array();
	//max length = 40 characters
	public function generateAlphaNumericPassword($length = 8){
		return substr(sha1(uniqid(rand(), true)), 0, $length);
	}
	function generateAlphaNumericPasswordMixed($length = 8)
	{
		$chars=array(
	...
	public function generateWordyPassword($numberOfWords=2,$maxDigitsBetweenWords=3,$allowDigitsAfterWords=false){
		$this->generatedPassword = '';
		if(!count($this->safeEnglishWords)){
			$this->loadSafeEnglistWords();
		}
		mt_srand ((float) microtime() * 1000000);

		$numSafeEnglishWords = count($this->safeEnglishWords);
		for ($i=0; $i < $numberOfWords; $i++) {
			$this->generatedPassword .= $this->safeEnglishWords[mt_rand(0,$numSafeEnglishWords)];
			if ($allowDigitsAfterWords || $i + 1 != $numberOfWords){
				$this->generatedPassword .= mt_rand(0,pow(10,$maxDigitsBetweenWords) -1);
			}
		}
		if ($debug) {
			$combinations = pow($numSafeEnglishWords, $numberOfWords) * pow(10, $maxDigitsBetweenWords * ($numberOfWords - 1));
	...
	private function loadSafeEnglistWords(){
		$this->safeEnglishWords = array( 'a', 'able', 'about', 'above', 'accept', 'accident', 'accuse', 'across', 'act',
